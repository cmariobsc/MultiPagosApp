    ...
    try{
        ...
        if($acreditacionId)
        {
            ...
            if($acreditacion)
            {
                ...
                $acreditacion->aprobada = $usuario->id;

                if($acreditacion->save())
                {
                    $empComercial = EmpComercial::where("empresa_id",$acreditacion->empresa_origen_id)->first();
                    $empComercial->credito += $acreditacion->valor;
                    ...
                    {
                        Capsule::rollback();
                        echo "0El saldo del cliente no pudo actualizarse, se cancela la operación";
                        exit();
                    }
                }
                else
